    ...
    <div className="min-h-full bg-slate-50">
      <div className="border-b border-slate-200 bg-slate-950 text-white">
        <div className="mx-auto max-w-7xl px-4 py-7 sm:px-6 lg:px-8">
          <div className="flex flex-col justify-between gap-5 md:flex-row md:items-end">
            <div>
              <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-sky-300"><Radar className="h-4 w-4" aria-hidden="true" /> Operations console</div>
              <h1 className="mt-2 text-3xl font-semibold tracking-tight">Autonomous bid fleet</h1>
              <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-300">Monitor system runs, policy exceptions, receiver mandates, evidence health, and submission outcomes across every customer project.</p>
            </div>
      ...
      <main className="mx-auto max-w-7xl px-4 py-7 sm:px-6 lg:px-8">
        <section aria-label="Portfolio health" className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {[
            { label: "Projects", value: projects.length, caption: "Latest 100 workspaces", icon: ListChecks, colour: "text-[#0A3D62]", bg: "bg-sky-50" },
            ...
            return <div key={item.label} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-center justify-between"><span className="text-xs font-medium text-slate-500">{item.label}</span><span className={`rounded-lg p-2 ${item.bg} ${item.colour}`}><Icon className="h-4 w-4" aria-hidden="true" /></span></div><div className="mt-3 text-3xl font-semibold tracking-tight text-slate-950">{item.value}</div><div className="mt-1 text-[11px] text-slate-500">{item.caption}</div></div>;
          })}
        </section>
                ...
                <tbody className="divide-y divide-slate-200">
                  {projects.map((project) => (
                    <tr key={project.id} className="group hover:bg-slate-50/80">
                      ...
                      <td className="px-5 py-4 text-right"><Link href={`/admin/projects/${project.id}`} className="inline-flex items-center gap-1 text-xs font-semibold text-[#0A3D62]">Inspect <ArrowRight className="h-3.5 w-3.5 transition group-hover:translate-x-0.5" aria-hidden="true" /></Link></td>
                    </tr>
                  ))}
                </tbody>
              </table>
