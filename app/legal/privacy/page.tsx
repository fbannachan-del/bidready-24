...
export default function PrivacyNotice() {
  return <LegalNotice title="Privacy notice">
    <p>BidReady 24 processes information to create and operate a source-cited tender preflight. The service is intended for business users handling procurement material they are entitled to use.</p>
    <h2>Information processed</h2><ul><li>Contact and support details, including name, business email and message content.</li><li>Payment status and identifiers received from Stripe. Payment-card data is handled by Stripe’s hosted checkout, not by the BidReady 24 application.</li><li>Tender documents, extracted text and file metadata.</li><li>Company facts, evidence documents and answers supplied through the project intake.</li><li>Analysis outputs, citations, confidence indicators, action history and technical audit records.</li><li>Hashed connection information used to limit repeated support-form abuse.</li></ul>
    ...
    <h2>Model training</h2><p>BidReady 24 does not intentionally use customer tender packs to train its own general-purpose models. When a configured model provider is used, its processing is governed by the applicable provider terms and the API controls described above.</p>
